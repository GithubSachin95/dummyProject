...
var choice = window.prompt("Hello Please enter your choice :" + '\n' + "1. Pokemon information "+ '\n' +  "2. Pokemon with same weaknesses" + '\n' +  "3. Pokemon which have their name in next evolution field");

var choice1 =parseInt(choice);

switch (choice1){

                case 1 :

                var enteredName = window.prompt("Enter pokemon name to get it's info")

                pokemonInfo(enteredName);
                ...
                var enteredName1 = window.prompt("Enter weakness to get list of pokemons who have same weaknesses");

                sameWeaknessPokemon(enteredName1);
                ...
                var enteredName2 = window.prompt("Enter pokemon name to get which other pokemons have their name in next evolution field")

                pokemonNextEvolution(enteredName2);
...
function pokemonInfo(_name){
        ...
        for(x in data){
                ...
                if(_name == data[x][i].name){

                    //console.log(data[x][i].name);

                    alert("id : " + data[x][i].id + '\n' +
                        ...
                        "height : " + data[x][i].height + '\n' +

                        "weight : " + data[x][i].weight + '\n' +

                        "weaknesses : " + data[x][i].weaknesses  );
...
//Function to determine which all pokemons have that name next evolution
function pokemonNextEvolution(_name){
	var pokemonDisplay =[];
	var evolvedPokemon = false
if(_name != "" && _name != undefined){
	for(x in data){
		for (i=0 ; i<= 150; i++){
			if(data[x][i].next_evolution != undefined){
				for(k=0;k<=1;k++){
				if(data[x][i].next_evolution[k] == undefined){
					k++;
					
					
				}else if( _name == data[x][i].next_evolution[k].name){
					pokemonDisplay.push(data[x][i].name);
					}
			}
			}
		...
		if(pokemonDisplay.length == 0){
			alert("No pokemon found with this name");
		}
		else{
		alert(pokemonDisplay);
		}
...
//function to find pokemons which share same weaknesses
function sameWeaknessPokemon(_name){
var pokemonList = [];
var pokemonList1 = [];
var emptyWeakness = false;
...
if(_name != "" && _name != undefined){
    ...
    for(x in data){
		...
		for(i=0;i<150;i++){
            ...
            for(j=0;j<=data[x][i].weaknesses.length;j++){

                if(_name == data[x][i].weaknesses[j]){
                    pokemonList.push(data[x][i].name)
                                                    }
													}
                                }
                }
                ...
                alert("Weakness can not be empty");
                emptyWeakness = true;
}
if(pokemonList.length == 0 && emptyWeakness != true){

                alert("No pokemon found with this weakness");
                }
                else if( pokemonList.length > 0 && emptyWeakness != true)
